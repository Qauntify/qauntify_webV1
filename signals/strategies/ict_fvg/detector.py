...
from signals.strategies.ict_smc.detector import (
    CHOCH_LOOKBACK,
    MAX_BARS_SINCE_CHOCH,
    MIN_SWEEP_ATR_FRACTION,
    PIVOT_LEFT,
    PIVOT_RIGHT,
    STRUCTURE_LOOKBACK,
    SWEEP_LOOKBACK,
...
MAX_STOP_ATR = 1.5
MIN_CANDLES = 25
# FVG retest must be fresh relative to the latest closed bar.
MAX_BARS_SINCE_RETEST = 2
...
def detect_setup(symbol, candles, atr14, htf_trend=None):
    ...
    `htf_trend` (typically 15m) gates direction when provided. No ADX and no
    ...
    # --- Long: sell-side liquidity sweep + bullish CHoCH + bullish FVG ---
    ...
    # --- Short: buy-side liquidity sweep + bearish CHoCH + bearish FVG ---
    for sweep_i in range(len(window) - 2, sweep_start - 1, -1):
        if htf_trend == "up":
            break
        _, swing_high = _recent_pivot_level(window, highs, sweep_i, kind="high")
